...
Loader::ResultStatus NCCHContainer::LoadHeader() {
    ...
    // NCCH header itself is plaintext; encrypted sections are decrypted on the fly in Load().
...
Loader::ResultStatus NCCHContainer::Load() {
    ...
    if (file->IsOpen()) {
        ...
        // On-the-fly NCCH decryption (restored from OG Citra; Azahar dropped support and rejected
        // any encrypted NCCH here). Keys come from the embedded default set / sysdata `keys.txt`.
...
Loader::ResultStatus NCCHContainer::LoadSectionExeFS(const char* name, std::vector<u8>& buffer) {
    ...
    for (unsigned section_number = 0; section_number < kMaxSections; section_number++) {
        ...
        if (strcmp(section.name, name) == 0) {
            ...
            // icon and banner use the primary key; .code and everything else the secondary key.
            std::array<u8, 16> key;
            ...
            if (strcmp(section.name, ".code") == 0 && is_compressed) {
                ...
                if (is_encrypted) {
                    dec.ProcessData(temp_buffer.data(), temp_buffer.data(), section_size);
                }
                ...
                if (exefs_file->ReadBytes(buffer.data(), section_size) != section_size)
                    return Loader::ResultStatus::Error;
                if (is_encrypted) {
                    dec.ProcessData(buffer.data(), buffer.data(), section_size);
                }
            }
...
Loader::ResultStatus NCCHContainer::ReadRomFS(std::shared_ptr<RomFSReader>& romfs_file,
    ...
    if (is_encrypted) {
        direct_romfs = std::make_shared<DirectRomFSReader>(
            std::move(romfs_file_inner), romfs_offset, romfs_size, secondary_key, romfs_ctr, 0x1000);
    } else {
        direct_romfs = std::make_shared<DirectRomFSReader>(std::move(romfs_file_inner), romfs_offset,
                                                           romfs_size);
    }
